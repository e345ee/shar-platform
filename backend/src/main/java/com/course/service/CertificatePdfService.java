...
public class CertificatePdfService {
    ...
    public byte[] generateCourseCertificate(Course course,
        ...
        try (PDDocument doc = new PDDocument();
            ...
            try (PDPageContentStream cs = new PDPageContentStream(doc, page)) {
                ...
                float maxTextWidth = width - 2 * margin - 40f;

                
                float y = height - margin - 65f;

                
                y = writeCenteredLine(cs, font, 18f, ORG_NAME, centerX, y);

                
                y = writeCenteredLine(cs, font, 44f, "СЕРТИФИКАТ", centerX, y);
                y = writeCenteredLine(cs, font, 16f, "о прохождении курса", centerX, y);

                
                String courseLine = "«" + courseName + "»";
                float courseFont = fitFontSize(font, courseLine, maxTextWidth, 28f, 16f);
                y = writeCenteredLine(cs, font, courseFont, courseLine, centerX, y);

                
                String studentLine = "Выдан: " + studentName;
                float studentFont = fitFontSize(font, studentLine, maxTextWidth, 16f, 12f);
                y = writeCenteredLine(cs, font, studentFont, studentLine, centerX, y);

                
                String teacherLine = "Преподаватель: " + teacher;
                float teacherFont = fitFontSize(font, teacherLine, maxTextWidth, 14f, 10f);
                y = writeCenteredLine(cs, font, teacherFont, teacherLine, centerX, y);

                
                y = writeCenteredLine(cs, font, 14f,
                        "Баллы за курс: " + earnedPoints + " из " + maxPoints,
                        centerX, y);

                
                String date = LocalDate.now().format(DateTimeFormatter.ofPattern("d MMMM yyyy", new Locale("ru")));
                y = writeCenteredLine(cs, font, 12f, "Дата выдачи: " + date, centerX, y);

                
                float footerY = margin + 22f;
                writeCentered(cs, font, 10f, "Этот сертификат сформирован автоматически в системе обучения.", width / 2f, footerY);
            }
    ...
    private float writeCenteredLine(PDPageContentStream cs,
        ...
        float leading = Math.max(14f, fontSize * 1.25f);
